...
interface BlogPostSummary {
  id: string;
  title: string;
  content: string; // ✅ instead of excerpt
  date: string;
  readTime: string;
  tags: string[];
  image: string | null;
...
export default function BlogListLayout({ initialSlug }: { initialSlug?: string }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // ✅ normalize + generate excerpt later in UI
      const data = raw.map((post: any) => ({
        id: post.id,
        title: post.title,
            ...
            <div className="p-4 pt-16">
              ...
              {loading ? (
                ...
                <BlogList 
                  posts={posts.map(p => ({
                    ...p,
                    excerpt: getExcerpt(p.content) // ✅ inject here
                  }))}
        ...
        <main className="w-full lg:w-2/3 overflow-y-auto p-4 lg:p-8 bg-gray-900/50 custom-scrollbar">
          ...
          {selectedSlug ? (
            ...
            <div className="text-center text-gray-500 p-10">
              Select a blog post from the list to view content.
            </div>
          )}
